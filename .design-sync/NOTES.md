...
- **分組方案（2026-07-31 更新）**：**「全頁・整頁改版底稿」排第一**（整頁快照，UI/UX 改版的入口）→ 再依裝置分「電腦版・〈頁面〉」×5（App 骨架／課程頁／待補課頁／學生頁／設定後台）→「手機版」→「共用・基礎」（tokens/字體/按鈕等通用件）。新卡的 group 照此格式；整頁快照一律進第一組，不要留在頁面組裡被元件淹掉。
  - 2026-07-31 同步：30 張卡（新上 `settings-page`／`courses-hero-options`／`calendar-day-view`／新做的 `students-page`），`style.css` 補推（雲端從 07-07 停在舊版、中間改過 8 個 commit）。
- **全頁卡的 modal 有第二條坑**：`.stu-modal` 的 `max-height:80vh`（`.week-modal` 沒這條）量的是瀏覽器視窗高，塞進 `.screen` 取景框會超出被 `overflow:hidden` 裁掉。解法＝preview CSS 補 `.screen .stu-modal{max-height:100%}`（wrap 是 inset:0+padding，100% 已扣留白）＋ 給開視窗那一格 `min-height`，整個視窗才一次看得完。往後做有 modal 的全頁卡都要檢查該 modal 有沒有 `vh` 尺寸。
